...
<body>
<header><div class="brand"><span class="mark">◫</span><span>TajikBlocks <span class="tag">барномасозии визуалӣ бо забони тоҷикӣ</span></span></div><button class="run" id="run">▶ Иҷро</button></header>
<main>
  <aside class="panel"><div class="head">Блокҳоро интихоб кунед</div><div class="hint">Блокро пахш кунед, баъд онро бо муш ҷобаҷо кунед.</div><div class="palette">
    <button data-type="print">🗨️ Навис</button><button data-type="var">✦ Тағйирёбанда</button><button data-type="if">◆ Агар</button><button data-type="loop">↻ Давр</button><button data-type="raw">⌨️ Рамзи ман</button>
  </div></aside>
  ...
  <section class="panel right"><div class="head">Рамзи TajikLang</div><div class="code-wrap"><textarea id="code" spellcheck="false" aria-label="Рамзи TajikLang"></textarea><div class="mini"><button id="copy">Нусха кардан</button><button id="reset">Аз нав</button></div></div><div class="output-wrap"><strong>Натиҷа</strong><div id="status" class="status">Омода</div><pre id="output" class="output">Натиҷаи барнома ин ҷо мебарояд.</pre></div></section>
</main>
<script>
const blocks=[], workspace=document.querySelector('#workspace'), code=document.querySelector('#code'), output=document.querySelector('#output'), status=document.querySelector('#status'); let dragging=null;
const esc=s=>String(s).replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));
const id=()=>Math.random().toString(36).slice(2);
function add(type){ const defaults={print:{text:'Салом Тоҷикистон!'},var:{name:'ном',value:'"Баҳром"'},if:{condition:'рост',text:'Шарт иҷро шуд'},loop:{start:'1',end:'5',text:'Рақам:'},raw:{text:'# Рамзи худро ин ҷо нависед'}}; blocks.push({id:id(),type,...defaults[type]}); render(); }
function source(b){ if(b.type==='print')return `навис(${JSON.stringify(b.text)})`; if(b.type==='var')return `бигзор ${b.name||'ном'} = ${b.value||'0'}`; if(b.type==='if')return `агар ${b.condition||'рост'}:\n    навис(${JSON.stringify(b.text)})`; if(b.type==='loop')return `барои i аз ${b.start||'1'} то ${b.end||'5'}:\n    навис(${JSON.stringify(b.text)}, i)`; return b.text||''; }
...
function render(){ workspace.innerHTML=''; if(!blocks.length){workspace.innerHTML='<div class="empty">Аз тарафи рост як блок интихоб кунед.<br>Рамз дар панел худкор пайдо мешавад.</div>'} blocks.forEach((b,index)=>{const el=document.createElement('article');el.className='block '+b.type;el.draggable=true;let fields=b.type==='print'?field('Матн','text',b.text):b.type==='var'?field('Ном','name',b.name)+field('Қимат','value',b.value):b.type==='if'?field('Шарт','condition',b.condition)+field('Агар рост бошад, навис','text',b.text):b.type==='loop'?field('Аз','start',b.start)+field('То','end',b.end)+field('Навис','text',b.text):field('Рамзи TajikLang','text',b.text,true);el.innerHTML=`<span class="handle">⠿</span><div class="fields">${fields}</div><button class="remove" title="Нест кардан">×</button>`;el.querySelectorAll('input,textarea').forEach(input=>{input.value=b[input.dataset.key];input.addEventListener('input',()=>{b[input.dataset.key]=input.value;code.value=makeCode()})});el.querySelector('.remove').onclick=()=>{blocks.splice(index,1);render()};el.addEventListener('dragstart',()=>dragging=index);el.addEventListener('dragover',e=>e.preventDefault());el.addEventListener('drop',e=>{e.preventDefault();if(dragging===null||dragging===index)return;const [moved]=blocks.splice(dragging,1);blocks.splice(index,0,moved);dragging=null;render()});workspace.appendChild(el)});code.value=makeCode(); }
document.querySelectorAll('[data-type]').forEach(button=>button.onclick=()=>add(button.dataset.type)); code.addEventListener('input',()=>{ status.textContent='Рамз дастӣ тағйир ёфт — метавонед онро иҷро кунед.'}); document.querySelector('#reset').onclick=()=>{blocks.length=0;output.textContent='Натиҷаи барнома ин ҷо мебарояд.';output.className='output';status.textContent='Омода';render()};document.querySelector('#copy').onclick=async()=>{await navigator.clipboard.writeText(code.value);status.textContent='Рамз нусха шуд.'};
